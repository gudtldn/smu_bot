...
class CourseModal(discord.ui.Modal, title="강좌 신청"):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.course_info = CourseInfo(course_name, description)

        guild_course_categories = {
            course.name[:-3]: None
        ...
        self.category_select = discord.ui.Select(
            options=[
                discord.SelectOption(label=name, value=name, default=(name == category))
                for name in guild_course_categories.keys()
            ],
        )
        self.category_label = discord.ui.Label(
            text="카테고리",
            description=f"{', '.join(guild_course_categories.keys())} 중에서 선택해주세요.",
            component=self.category_select,
        )

        self.add_item(self.course_info.course_name)
        self.add_item(self.category_label)
        self.add_item(self.course_info.description)
        self.add_item(self.course_info.agree)

    async def on_submit(self, interaction: discord.Interaction["Bot"]):
        selected_category_name = self.category_select.values[0]

        def get_view():
            return CourseModalView(category=selected_category_name, **self.course_info)
        ...
        selected_category = course_category.get(selected_category_name) or course_category['기타']

        # 기존 카테고리 권한 상속 + 신청자에게 채널 관리 권한 부여
        channel_overwrites = selected_category.overwrites | {
    ...
    async def on_error(self, interaction: discord.Interaction["Bot"], error: Exception) -> None:
        category_values = self.category_select.values
        await interaction.response.send_message(
            "강좌를 만들던 도중 에러가 발생하였습니다. 다시 시도해 주세요.",
            view=CourseModalView(category=category_values[0] if category_values else None, **self.course_info),
            ephemeral=True
        )
        error_msg = (
            ...
            f"카테고리: {category_values[0] if category_values else "선택 없음"}\n"
